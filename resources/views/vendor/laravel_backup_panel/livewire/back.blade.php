<div class="container mb-5">
    <div class="d-flex align-items-end pt-2">
        <h5 class="mb-0">
            Panel de respaldos del sistema
        </h5>

        <button id="create-backup" class="btn elevation-4 bg-navy btn-sm ml-auto">
            Crear respaldo del sistema
        </button>
        <button class="btn bg-navy elevation-4 btn-sm ml-auto px-3" href="#" id="create-backup-only-db" wire:click.prevent="">
            Crear repaldo de la base de datos
        </button>
        <button class="btn bg-navy elevation-4 btn-sm ml-auto px-3" href="#" id="create-backup-only-files" wire:click.prevent="">
        ...
        <div class="col-12">
            <div class="card shadow-sm mb-4">
                <div class="card-header d-flex align-items-end">
                    <button class="btn bg-navy elevation-5 btn-sm btn-refresh ml-auto"
                    ...
                    @if(count($disks))
                        <div class="btn-group btn-group-toggle" data-toggle="buttons">
                            @foreach($disks as $disk)
                                <label class="btn bg-navy elevation-5 {{ $activeDisk === $disk ? 'active' : '' }}"
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-body">
                                <h5 class="modal-title mb-3">Borrar respaldo</h5>
                                @if($deletingFile)
                                <span class="text-muted">
                                    Está seguro que desea borrar el respaldo creado el  {{ $deletingFile['date'] }} ?
                                </span>
                                @endif
                            </div>
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn bg-navy elevation-5 delete-button" wire:click="deleteFile">Borrar</button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('livewire:load', function () {
            ...
            const backupFun = function (option = '') {
                Toastify({
                    text: 'Creando una nueva copia de seguridad en segundo plano...' + (option ? ' (' + option + ')' : ''),
                    duration: 5000,
                    gravity: 'bottom',
                    position: 'right',
